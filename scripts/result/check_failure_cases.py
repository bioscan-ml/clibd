...
FAILURE_CASE_FOLDER = "failure_case_analysis"
...
def failure_case_analysis_for_specific_query_and_key(args,
                                                     name,
                                                     query_dict,
                                                     keys_dict,
                                                     pred_dict,
    ...
    if failure_cases is not None:
        if len(failure_cases[query][key]) > num_queries:
            query_indices = failure_cases[query][key][0:num_queries]
        else:
            query_indices = failure_cases[query][key]
        if len(query_indices) == 0:
            print(f"No failure cases found for {query} to {key} in {failure_level}")
            return None
    else:
    ...
    folder_path = os.path.join(folder_path, f"{query}_to_{key}")
    os.makedirs(folder_path, exist_ok=True)
    retrieved_images_json_path = os.path.join(folder_path,
                                              f"failure_analysis_in_{failure_level}.json")
    if load_cached_results and os.path.exists(retrieved_images_json_path) and False:
    ...
    if not independent:
        fig.tight_layout()
        path_to_save = os.path.join(folder_path, f'failure_analysis_in_{failure_level}.pdf')
        fig.savefig(path_to_save, transparent=True, bbox_inches="tight")
        print(f"Saved retrieved images to {path_to_save}")
    ...
    print(f"Saved retrieval results to {retrieved_images_json_path}")
...
def failure_case_analysis(
    ...
    """
    # Find query and key index with their top-1 prediction is wrong


    for failure_level in ["species", "genus", "family", "order"]:
        failure_cases = {}
        for query, key in QUERY_AND_KEY_WE_CARE_ABOUT:
            if query not in failure_cases.keys():
                failure_cases[query] = {}
            if key not in failure_cases[query].keys():
                failure_cases[query][key] = []
            for query_idx, query_gt_labels in enumerate(query_dict['label_list']):
                if seen_or_unseen == 'seen':
                    curr_prediction = pred_dict[query][key]['curr_seen_pred_list'][query_idx][failure_level][0]
                else:
                    curr_prediction = pred_dict[query][key]['curr_unseen_pred_list'][query_idx][failure_level][0]
                if query_gt_labels[failure_level] != curr_prediction:
                    failure_cases[query][key].append(query_idx)
            print(f"Number of failure cases for {query} to {key} is {len(failure_cases[query][key])}")

        for query, key in QUERY_AND_KEY_WE_CARE_ABOUT:
            failure_case_analysis_for_specific_query_and_key(
                args,
                name,
                query_dict,
                keys_dict,
...
def main(args: DictConfig) -> None:
    ...
    if args.load_inference and os.path.exists(per_claSS_acc_path) and os.path.exists(acc_dict_path) and os.path.exists(
            pred_dict_path):
        with open(per_claSS_acc_path, "r") as json_file:
            per_class_acc = json.load(json_file)

        with open(acc_dict_path, "r") as json_file:
            acc_dict = json.load(json_file)

        with open(pred_dict_path, "r") as json_file:
            pred_dict = json.load(json_file)
        print("Loaded inference and evaluation results from file...")
    else:
        ...
        print("Done inference and evaluation!")

        # save acc_dict and per_class_acc
